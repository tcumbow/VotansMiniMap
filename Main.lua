...
function addon:InitMiniMap()
	...
	do
		...
		function ZO_WorldMapButtonsFloors:SetHidden(hidden)
			return orgSetHidden(self, (not addon.account.allowFloorNavigation and ZO_WorldMap_GetMode() == MAP_MODE_VOTANS_MINIMAP) or hidden)
		end
	end
	do
...
do
	...
	function addon:StepZoom(add)
		if ZO_WorldMap_GetMode() ~= MAP_MODE_VOTANS_MINIMAP or WORLD_MAP_FRAGMENT:IsHidden() or addon.isSpecialZoom or not self.account[self.zoomMode] then
